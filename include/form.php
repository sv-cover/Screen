...
class InputField extends Field
{
    ...
    public function render(array $attributes=[]) {
        ...
        if (isset($this->value))
            $attributes['value'] = $this->render_value();
...
class DateField extends InputField
{
    ...
    public function validate() {
        if ($this->optional && $this->value === '')
            return true;
        else if (date_parse_from_format('Y-m-d', $this->value)['error_count'] === 0) {
...
/**
 * DateTimeField: An class for a HTML input field with type="datetime-local"
 */
class DateTimeField extends InputField
{
    public function __construct() {
        $args = func_get_args();
        array_unshift($args, 'datetime-local');
        call_user_func_array(['parent', '__construct'], $args);
    }
    ...
    public function validate() {
        if ($this->optional && $this->value === '')
            return true;
        else if (date_parse_from_format('Y-m-d\TH:i', $this->value)['error_count'] === 0) {
            $this->value = DateTime::createFromFormat('Y-m-d\TH:i', $this->value);
            return true;
        }
        ...
        if ($this->value === '' )
            ...
            $this->errors[] = sprintf('Please enter a valid date and time');
...
class NumberField extends InputField
{
     ...
     */
    public function validate() {
        if (!$this->optional && (!isset($this->value) || (empty(trim($this->value)) && $this->value !== '0')) ) {
            $this->errors[] = sprintf('%s is required', $this->label);
        ...
        elseif (array_key_exists('min', $this->attributes) && $this->value < $this->attributes['min'])
            $this->errors[] = sprintf('Please enter a number larger than %d', $this->attributes['min']);
        elseif (array_key_exists('max', $this->attributes) && $this->value > $this->attributes['max'])
...
/**
 * DateField: An class for a HTML input field with type="url"
 */
class UrlField extends InputField
{
    ...
    public function validate() {
        ...
        if ($value === '' )
            ...
            $this->errors[] = sprintf('Please enter a valid url');
