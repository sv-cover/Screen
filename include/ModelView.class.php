...
class ModelView extends FormView
{
    ...
    /**
     * Runs the csv view 
     * Creates the output CSV, sends the appropriate headers, the CSV data and 
     * terminates execution. This view is disabled by default.
     */
    protected function run_csv() {
        $data = $this->get_csv_data();
        
        header('Content-type: text/csv');
        header('Content-disposition: attachment;filename=' .  $this->page_id . '.csv;');
    ...
    /** Returns an associative array with data to use in the CSV export */
    protected function get_csv_data() {
        return $this->get_model()->get();
    }
    ...
    protected function get_object() {
        ...
        if (!isset($this->_object)){
            $this->_object = $this->get_model()->get_by_id($_GET['id']);
